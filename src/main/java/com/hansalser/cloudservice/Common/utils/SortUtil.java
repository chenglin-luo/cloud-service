package com.hansalser.cloudservice.Common.utils;
...
public class SortUtil {
    ...
    public static void handlePageSort(QueryRequest request, Page page, String defaultSort, String defaultOrder, boolean camelToUnderscore) {
        ...
        if (camelToUnderscore) {
            sortField = CloudServiceUtil.camelToUnderscore(sortField);
            defaultSort = CloudServiceUtil.camelToUnderscore(defaultSort);
        }
        if (StringUtils.isNotBlank(request.getField())
            ...
            if (StringUtils.equals(request.getOrder(), CloudServiceConstant.ORDER_DESC))
                page.setDesc(sortField);
            else
                page.setAsc(sortField);
        } else {
            if (StringUtils.isNotBlank(defaultSort)) {
                if (StringUtils.equals(defaultOrder, CloudServiceConstant.ORDER_DESC))
