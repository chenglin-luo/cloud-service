...
public class ShiroRealm extends AuthorizingRealm {
    ...
    protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principal) {
        ...
        Set<String> roleSet = roleList.stream().map(RoleDO::getRoleName).collect(Collectors.toSet());
        simpleAuthorizationInfo.setRoles(roleSet);

        // 获取用户权限集
        List<MenuDO> permissionList = this.menuService.findUserPermissions(userName);
        Set<String> permissionSet = permissionList.stream().map(MenuDO::getPermission).collect(Collectors.toSet());
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken token) throws AuthenticationException {
        ...
        // 通过用户名到数据库查询用户信息
        UserDO user = this.userService.findByName(userName);
